...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar 28 20:55:21 2022
...
def my_fun(n=1):
    ...
    while True:
        ...
        #conformal=(abs(m.muh2/v**2)<1e-2) and (abs(m.mus2/v**2)<1e-2)
        m.print_couplings()
        Pih=g1**2/16 + 3*g**2/16 + m.lamh/2 + 1/4 + m.lammix/24
        Pis=m.lammix/6 + m.lams/4
        lamh_tilde=m.lamh - m.lammix**2/4/m.lams
        if thbool==True and (lamh_tilde<0.01):
            break



    dict_out={'ms':m.ms,'theta':m.theta, 'u':m.u,"lamh":m.lamh,"lams":m.lams,"lammix":m.lammix,
